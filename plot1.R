...
plot1 <- function(in_file = "household_power_consumption.txt", i_date_str = "2007-02-01", e_date_str= "2007-02-02") {
    ...
    data_raw$Date_time <- strptime(paste (data_raw$Date,data_raw$Time), format = "%d/%m/%Y %T" )
    ...
    with (d_raw, hist(Global_active_power, main = "Global Active Power", col = "red", xlab = "Global Active Power (kilowatts)"))
    dev.off()
